...
stockData<-merge(stockData,stickerData,by.x="symbol",by.y="Symbol")

stockData$Sector<-as.factor(stockData$Sector)
...
arules.model<-apriori(trans,parameter = list(support=.02, confidence=.3, minlen=2))
